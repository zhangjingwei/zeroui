 ...
 * - [onResponse] MUST be invoked on the main thread (so reducer state writes are safe).
 *   Implementations should also honour cancellation: if [Cancelable.cancel] was invoked
 *   before the response landed, [onResponse] should NOT fire.
...
fun interface HttpClient {
    ...
    companion object {
        ...
        val Noop: HttpClient = HttpClient { _, _, _, _, _ -> Cancelable.Noop }
    }
}
