...
ZeroUI 是一个面向 Android 的轻量级 Server-Driven UI 运行时，通过 JSON 协议描述页面结构、交互逻辑与设计 token，在 Jetpack Compose 中渲染原生界面，无需修改客户端代码即可更新 UI。适合搭建数据列表与详情页、动态表单与多步骤向导、搜索与过滤页、运营活动页，以及各类配置和业务工具界面，同时保留原生性能、完整的状态管理、HTTP 数据驱动和宿主能力接入。
